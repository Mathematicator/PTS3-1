...
public class NetworkSaveData {
    ...
    public NetworkSaveData(Context context) {
        ...
        ArrayList<Note> notes = noteManager.recupererNotesNonSynchroniser();
        if(notes != null) {
            for (Note n : notes) {
        ...
        NoteDetailsManager noteDetailsManager = new NoteDetailsManager(context);
        ArrayList<NoteDetails> noteDetails = noteDetailsManager.recupererTout();
        if(noteDetails != null) {
            for (NoteDetails n : noteDetails) {

                String q = "INSERT INTO `NOTE_DETAILS` (`id`,`details`,`id_note`) VALUES ('" + n.getId() + "', '" + n.getDetails() + "', '" + n.getId_note() + "');";
